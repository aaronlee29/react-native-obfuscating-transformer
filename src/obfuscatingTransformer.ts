  ...
  return {
    transform(props) {
      ...
      if (resultCanBeObfuscated && filter(props.filename, props.src)) {
        ...
        const transform =  maybeTransformMetroResult(
          result,
          obfuscateCodePreservingSourceMap(
            code,
            map,
        ...
        if(otherOptions.emitObfuscatedFiles || otherOptions.clearObfuscatedFiles) {
          const emitDir = path.dirname(props.filename)
          const filename = extendFileExtension(
              path.basename(props.filename),
              "obfuscated",
          )
          const emitPath = path.join(emitDir, filename)

          if (otherOptions.emitObfuscatedFiles) {
            fs.writeFileSync(emitPath, transform)
          } else {
            if(otherOptions.clearObfuscatedFiles) {
